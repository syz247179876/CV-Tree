"""
YoloV8 baseline
"""
from ultralytics.models import YOLO
from ultralytics.alchemy.settings import *
from ultralytics.utils.wraps import log_profile

@log_profile('thop', shape=(3, 640, 640), batch_size=2, show_detail=True)
def get_model(model: str, device: str = 'cuda'):
    model = YOLO(model)
    return model

if __name__ == '__main__':
    model = get_model(r'yolov8-SPPF-Avg-Attn.yaml')
    # model.train(data=DATASET_VOC, epochs=50, batch=16, lr0=0.001, name=f'train-{DATASET_VOC}-baseline-v8n-b16-prune', resume=False)