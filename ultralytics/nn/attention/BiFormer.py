...
__all__ = ['BiLevelRoutingAttention', 'BiFormerBlock', 'BRABlock', 'BiPatchMerging', 'BiStem', 'BiFormerBlocks']
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, n_win=7, trans=False, qk_dim=None, qk_scale=None,
        ...
        # 粗粒度区域筛与每个Query最相关的K个key-value pairs
        ...
        # 离散的K个key-value pairs重组, 使得K个key 和 value在内存上连续
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        # 为了简化运算, 聚合每个区域的所有像素点, 记作q_i, q_i代表了每个region的像素点的特征，这里用mean，相当于在每个region的像素维度做GAP
        q_win, k_win = q.mean([2, 3]), kv[..., 0:self.qk_dim].mean(
            [2, 3])  # window-wise qk, (n, p^2, c_qk), (n, p^2, c_qk)

        ##################side_dwconv(lepe)##################
        # 在value向量上增加局部卷积, 增加局部上下文信息
        ...
        # 粗粒度筛选出每一个Query关注度最高的前K个key-value pairs
        r_weight, r_idx = self.router(q_win, k_win)  # both are (n, p^2, topk) tensors

        # 重组前K个key-value pairs, 使之在内存上连续
        ...
        # 相比如传统的Transformer的自注意力计算的细粒度更小
...
class BRABlock(nn.Module):
    ...
    def __init__(self, dim, out_dim, num_heads=8, n_win=7,
                 qk_scale=None, topk=4, mlp_ratio=4, side_dwconv=5, drop_path=0.,
        ...
        if topk > 0:
            self.attn = BRA(dim=dim, num_heads=num_heads, n_win=n_win,
    ...
    def __init__(self, dim, out_dim, n_win, drop_path=0., layer_scale_init_value=-1,
        ...
        if topk > 0:
            self.attn = BiLevelRoutingAttention(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                                                ...
                                                auto_pad=auto_pad)
            # self.attn = BRA(dim=dim, num_heads=num_heads, n_win=n_win, qk_scale=qk_scale, topk=topk,
            #                 side_dwconv=side_dwconv)
        elif topk == 0:
...
class BiFormerBlocks(nn.Module):

    def __init__(self, dim, out_dim, depth: int, num_heads=8, n_win=7,
                 topk=4, mlp_ratio=4, side_dwconv=5, drop_path=0.,
                 norm_layer=LayerNorm2d, qk_scale=None,):
        super(BiFormerBlocks, self).__init__()
        blocks = []
...
class BiStem(nn.Module):
    """
    PatchEmbedding of BiFormer
    """

    def __init__(self, in_chans: int, out_chans: int):
        super(BiStem, self).__init__()
        self.in_chans = in_chans
        self.out_chans = out_chans
        self.stem = nn.Sequential(
            Conv(in_chans, out_chans // 2, k=3, s=2, act=nn.GELU()),
            Conv(out_chans // 2, out_chans, k=3, s=2, act=False)
        )
...
class BiPatchMerging(nn.Module):

    """
    PatchMerging of BiFormer
    """
    ...
    def __init__(
        ...
        super(BiPatchMerging, self).__init__()
        self.down_padding = Conv(in_chans, out_chans, k=kernel_size, s=stride)
