...
class CloConv(nn.Module):
    ...
    def __init__(
        ...
        super(CloConv, self).__init__()
        self.conv = nn.Conv2d(in_chans, out_chans, kernel_size, stride, autopad(kernel_size, padding, dilation),
...
class PatchEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            Conv(in_chans, out_chans // 2, k=3, s=2),
            Conv(out_chans // 2, out_chans, k=3, s=2),
            Conv(out_chans, out_chans, k=3, s=1),
            Conv(out_chans, out_chans, k=3, s=1),
...
class ConvFFN(nn.Module):
    ...
    def __init__(
        ...
        self.act = act_layer()
        self.dwc = nn.Conv2d(hidden_chans, hidden_chans, kernel_size, stride, autopad(kernel_size),
                             groups=hidden_chans)
