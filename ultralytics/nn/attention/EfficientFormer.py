...
__all__ = ['Stem', 'MetaBlock', 'PatchMerging', 'Conv2dBN']
...
class Conv2dBN(torch.nn.Sequential):
    ...
    def __init__(self, in_chans: int, out_chans: int, ks=1, stride=1, dilation=1,
        ...
        self.add_module('c', torch.nn.Conv2d(
            in_chans, out_chans, ks, stride, autopad(ks, d=dilation), dilation, groups, bias=False))
...
class Stem(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            Conv(in_chans, out_chans // 2, k=3, s=2, act=nn.ReLU(inplace=True)),
            Conv(out_chans // 2, out_chans, k=3, s=2, act=nn.ReLU(inplace=True)),
        )
...
class MB4D(nn.Module):
    ...
    def __init__(
        ...
        self.token_mixer = nn.AvgPool2d(kernel_size=pool_size, stride=1, padding=autopad(k=pool_size),
