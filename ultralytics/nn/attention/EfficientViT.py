...
from ultralytics.nn.modules.utils import auto_pad
...
class PatchEmbedSmall(nn.Module):
    ...
    def __init__(
        ...
        if act_layer is None:
            act_layer = nn.ReLU
        self.patch_embed = nn.Sequential(
            Conv2dBN(in_chans, embed_dim // 4, ks=3, stride=2, resolution=resolution),
            act_layer(inplace=True),
            Conv2dBN(embed_dim // 4, embed_dim // 2, ks=3, stride=2, resolution=resolution // 2),
            act_layer(inplace=True),
            Conv2dBN(embed_dim // 2, embed_dim, ks=3, stride=2, resolution=resolution // 4),
        )
    ...
    def __init__(
        ...
        for i, (stage_mode, embed_dim, depth, num_head, window_size, key_dim, hidden_ratio) in enumerate(
            ...
            if i < 2 and down_ops[0] == 'sandwich':
                ...
                # resolution_ = resolution // down_ops[1]
